...
ui <- navbarPage(  
  ...
  header = tags$head(  
    ...
    tags$style(HTML("  
      ...
      .well { background-color: #f8f9fa; border-radius: 10px; }  
    ...
    fluidRow(  
      column(  
        ...
        wellPanel(  
          ...
          numericInputIcon(  
            "Hb",  
            label = tags$span(icon("tint"), "Hemoglobin (g/L)"),  
            value = 130,  
            min = 50,  
            max = 200,  
            step = 1,  
          ...
          numericInputIcon(  
            "WBC",  
            label = tags$span(icon("microscope"), "White Blood Cell Count (10⁹/L)"),  
            value = 6.5,  
            min = 1,  
            max = 30,  
            step = 0.1,  
        ...
        wellPanel(  
          ...
          div(  
            ...
            tags$ul(  
              ...
              tags$li("Hb (g/L)"),  
              tags$li("White Blood Cell Count (10⁹/L)")  
            ),  
            br(),  
            fileInput(  
              "batch_file",  
      ...
      column(  
        ...
        div(  
          ...
          tags$ul(  
            ...
            tags$li("Features: ALT, Age, BMI, Gender, Hb, White Blood Cell Count"),  
            tags$li("Performance: AUC = 0.8001, Accuracy = 73.91%"),  
          ...
          p("This tool is intended for clinical research and screening purposes. It should not replace clinical judgment or comprehensive geriatric assessment."),  
          hr(),  
          div(  
            class = "text-center",  
...
server <- function(input, output, session) {  
  ...
  predict_single_patient <- function(params) {  
    tryCatch({  
      ...
      input_data <- data.table(  
        ...
        Hb = as.numeric(params$Hb),  
        WBC = as.numeric(params$WBC),  
      ...
      key_drivers_df <- compute_key_drivers(  
        ...
        X_new = input_data[, model_components()$feature_cols, with = FALSE],  
  ...
  predict_batch_patients <- function(data, include_drivers = TRUE) {  
    tryCatch({  
      required_cols <- c("ALT", "Age", "BMI", "Gender", "Hb", "WBC")  
      ...
      numeric_cols <- c("ALT", "Age", "BMI", "Hb", "WBC")  
      ...
      results <- data.table(  
        ...
        Hb = data_processed$Hb,  
        WBC = data_processed$WBC,  
      ...
      if(include_drivers) {  
        ...
        key_drivers_df <- compute_key_drivers(  
          learner = model_components()$learner,  
          X_new = data_processed[, model_components()$feature_cols, with = FALSE],  
          bg_X = model_components()$background_data,  
          feature_cols = model_components()$feature_cols,  
          target_levels = model_components()$target_levels  
        )  
      ...
      if(n_patients > 0) {  
        descriptive_stats <- list(  
          ...
          Hb = summary(data_processed$Hb),  
          WBC = summary(data_processed$WBC)  
        )  
      } else {  
    ...
    fluidRow(  
      ...
      column(6,  
             ...
             renderPrint({  
               ...
               gender_counts <- desc$Gender_dist  
               names(gender_counts) <- gender_labels[as.numeric(names(gender_counts))]  
               print(gender_counts)  
               
               cat("\nRisk Level Distribution:\n")  
               risk_counts <- table(values$batch_results$RiskLevel)  
               print(risk_counts)  
             })  
      )  
    )  
    ...
    tagList(  
      div(  
        ...
        fluidRow(  
          ...
          column(2, strong("Hb:"), br(), result$features$Hb, " g/L"),  
          column(2, strong("WBC:"), br(), result$features$WBC, " ×10⁹/L")  
        )  
      )  
    )  
  })  
